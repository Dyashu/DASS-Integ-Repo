...
async function fetchPageData(index) {
  try {
    const response = await fetch(`/api/pages/${index.index}`, {
      // Use template literals to include the index in the URL
      method: "GET",
      headers: {
        ...
        {kontent.map((konts) => (
          <div
            className={`sub-nav-links ${
              dropdownStates[konts.index] ? "dropdown-open" : ""
            }`}
            key={konts.index}
          >
              ...
              {konts.subidS.length !== 0 ? (
                <>
                  <span
                    ...
                    onClick={() => DisplayPage(konts.index)}
                  >
                    {konts.title}
                  </span>
                  <div
            ...
            {dropdownStates[konts.index] && subPages[konts.index] ? (
              <SubPages kontent={subPages[konts.index]} />
            ) : (
              <></>
            )}
          </div>
    ...
    <>
      <div className="navbar">
        <div className="left-section">
          ...
          <div className="menu-name-literally">IT Services</div>
        </div>
      ...
      <nav className={`side-bar ${isOpen ? "open" : ""}`}>
        ...
        <div className="nav-links">
          {pageTitles.map((page) => (
            <div
              className={`nav-item ${
                dropdownStates[page.index] ? "dropdown-open" : ""
              }`}
              key={page.index}
            >
              <div className="nav-name">
                {page.subidS.length !== 0 ? (
                  <>
                    <span
                      style={{ marginLeft:"4vw" }}
                      onClick={() => DisplayPage(page.index)}
                    >
                      {page.title}
                    </span>
                    <div
              ...
              {dropdownStates[page.index] && subPages[page.index] ? (
                <SubPages kontent={subPages[page.index]} />
              ) : (
                <></>
              )}
            </div>
